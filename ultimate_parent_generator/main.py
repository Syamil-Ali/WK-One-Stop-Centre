...
# layout
import ultimate_parent_generator.layout.custom_html as ch
import ultimate_parent_generator.layout.header as header

# function
import ultimate_parent_generator.function as func

from starlette.formparsers import MultiPartParser
MultiPartParser.spool_max_size = 1024 * 1024 * 2048 #2gb

@ui.page('/ultimate_parent_generator')
async def main_app():


    try:
        await ui.context.client.connected(timeout=300.0) # <<< this line is critical
    except TimeoutError:
    ...
    if 'df_work' not in app.storage.tab:
        app.storage.tab['df_work'] = None 
    ...
    async def excel_upload_handler(event: events.UploadEventArguments):
        ...
        try:
            ...
            app.storage.tab['df_work'] = app.storage.tab['excel_sheets_dict'].get("Ultimate Parent Generator")
            ...
            if len(app.storage.tab['excel_sheets_dict']) > 0:
                ...
                func.start_generate_ultimate_parent()

                # export the file
                func.export_excel_work(app.storage.tab['df_work'], app.storage.tab['today_str'])
                ui.notify('Exported!')
    ...
    with ui.element('div').style('height: 80vh;').classes('flex pt-[20vh] justify-center max-w-[800px] w-full mx-auto'):
        with ui.column().classes('items-center w-full gap-4'):
            ...
            # Title
            ui.label('ULTIMATE PARENT ACCOUNT GENERATOR') \
