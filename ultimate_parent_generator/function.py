...
import tempfile
from ultimate_parent_generator.func import ultimate_parent_generator as upg
import requests
...
def read_excel_sheets(content): # Predefined Opportunity Generator Tab
    ...
    sheet_names = ['Ultimate Parent Generator','Account Object']
    result_dict = {}
...
def export_excel_work(uploaded_file, today_date):
    
    if uploaded_file is not None:
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.csv') as tmp:
            uploaded_file.to_csv(tmp.name, index=False)
            tmp.flush()
            ui.download(tmp.name, filename=f'result-ultimate-parent-generator-{str(today_date)}.csv')
    else:
        ui.notify('No file to download')


# PROCESS MQL
def start_generate_ultimate_parent():

    df_acc= app.storage.tab['excel_sheets_dict']['Account Object']

    app.storage.tab['df_work'] = upg.get_ultimate_parent_ce_number(app.storage.tab['df_work'], df_acc)
