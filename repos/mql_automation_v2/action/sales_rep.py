...
def get_location_zipcode(zipcode, wk_territory, total_item, product):
    ...
    PATIENT ENGAGEMENT – Follow UTD Logi
    ...
    try:
        ...
        # 1. Ensure the zipcode variable is a string (as you already did)
        zipcode = str(int(zipcode))

        # 2. CRITICAL STEP: Convert the 'Postal Code' column in the DataFrame
        #    to string type to match the 'zipcode' variable type.
        wk_territory['Postal Code'] = wk_territory['Postal Code'].astype(str)

        # 3. Now, perform the filter: the types match, and it should work.
        acc_info = wk_territory[wk_territory['Postal Code'] == zipcode].copy()
        acc_info2 = wk_territory[wk_territory['Postal Code'] == int(zipcode)].copy()



        if (product.strip().upper() == 'UPTODATE') or (product.strip().upper() == 'EMMI'):
            ...
            if total_item <= utd_growth:
                ...
                territory = 'more than threshold, notify Matt, Don and Ron McBride'
                region = 'more than threshold, notify Matt, Don and Ron McBride'
...
def main_sales_rep(row, account_object, wk_territory, wk_new_assignment):
    ...
    try:
        ...
        # get extras
        if (str(row['PRODUCT']).strip().upper() == 'UPTODATE') or (str(row['PRODUCT']).strip().upper() == 'EMMI'):

            value = number_type(row['Number Clinicians'])

            territory_extras, region_extras = get_location_zipcode(row['ZipCode'], wk_territory, value, row['PRODUCT'])
        ...
        try:
            if (str(row['PRODUCT']).strip().upper() == 'UPTODATE') or (str(row['PRODUCT']).strip().upper() == 'EMMI'):
                ...
                territory_extras, region_extras = territory, region
    ...
    # do for extras
    sales_rep_extras, acc_owner_extras = get_sales_rep(row['PRODUCT'], wk_new_assignment, territory_extras)



    return territory, region, sales_rep, acc_owner, territory_extras, region_extras,sales_rep_extras, acc_owner_extras
