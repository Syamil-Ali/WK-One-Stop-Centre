...
async def main_app():
    ...
    if 'excel_sheets_name' not in app.storage.tab:
        app.storage.tab['excel_sheets_name'] = []
    ...
    async def excel_upload_handler(event: events.UploadEventArguments):
        ...
        try:
            ...
            app.storage.tab['excel_sheets_name'] = await run.io_bound(func.check_excel_sheets, content) # check sheets
            
            spinner_container.clear()
            
            if len(app.storage.tab['excel_sheets_name']) > 0:
                #print('pass sheet name')


                app.storage.tab['opp_generator_file'] = await run.io_bound(func.read_excel_sheet_mini, content,'Opportunity Generator')
                #print('pass read opp generator')

                if app.storage.tab['opp_generator_file'] is not None:
                    #print('pass read opp generator')
                    ...
                    app.storage.tab['opp_generator_file'] = await run.io_bound(func.start_process_opportunity, app.storage.tab['opp_generator_file'], content)
                    #print('pass process opp')
